...
> **改訂（2026-09-23）** — 複数旅行に対応し、D4・D8 の単一旅行という前提を変更した。
> 現行スキーマは [000004_multiple_trips.up.sql](../../backend/migrations/000004_multiple_trips.up.sql) と
> [erd.mmd](erd.mmd) を参照。`trip.id` は BIGINT AUTO_INCREMENT、`slug` は一意の URL キー、
> `theme` は `okinawa` / `onsen`。`days.trip_id` から `trip` を参照し、日付は
> `UNIQUE(trip_id,date)` とする。既存行は `okinawa` に所属させる。
> 以下の意思決定と初期DDLは変更前の設計記録。現在は `trip → days → items` の関係を持ち、
> 日付や予定の操作は選択中の旅行に限定する。ログイン・利用者テーブルは設けない。
...
当時は「ふたりの記念日」による入口の演出をフロントだけで残していたが、
複数旅行に対応した 2026-09-23 にログイン画面ごと廃止した(`ANNIVERSARY` 定数も削除)。
